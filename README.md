...
You may also notice that although the project is still called "Orabuntu-LXC" the logo however now has "Orabuntu-LX**D**" to draw attention to the fact that the Orabuntu-LXC software now gives has the option with the push of "one button" N-node LXD Clusters across multiple hosts. Alternatively,the original LXC deployment mode which allows to spans of LXC container networks across multiple hosts (not a true "cluster" however, only a "span").
