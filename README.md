...
Otherwise, that's it. It runs fully-automated and delivers LXC single network spanned via OpenvSwitch across multiple hosts, **or now alternatively an LXD cluster**, container infrastructure over OpenvSwitch. There are optional switches in the CONFIG file to deploy Docker snap and microk8s snap.  Currently, the LXD cluster features of Orabuntu-LXC only supports ZFS storage pools.  
