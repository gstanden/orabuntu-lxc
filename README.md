...
Announcing Orabuntu-LXC v7.0.0-alpha ELENA (Enterprise LXD Edition New AMIDE) available now (in MASTER branch only) introducing support for fully-automated N-node LXD cluster creation. The github will continue to be known as "Orabuntu-LXC" but the logo has been updated with "Orabuntu-LXD" to communicate LXD cluster support.

New features of v7.0.0-alpha ELENA edition:

* Automated LXD N-node clusters on OpenvSwitch VLAN's on CentOS, Fedora, RedHat, Ubuntu and Oracle Linux.
...
* CentOS 6.x, 7.x, 8.x
* Fedora 22-34+ (tested on 22, 24, 29, 33 and 34)
* RedHat 6.x, 7.x, 8.x
...
To add other flavors of LXD container to the Orabuntu-LXC DHCP OpenvSwitch networks, after Orabuntu-LXC scripts have finished, and the system is configured, simply choose the profile desired and then launch the container.  There are two user-settable networks created as previously mentioned.  The default networks for this example are 10.209.53.0/24 and 172.29.108.0/24 (but these are user-settable and can be changed prior to running Orabuntu-LXC they are BaseNet and SeedNet, respectively) which have LXD profiles olxc_sw1a and olxc_sx1a respectively.  So for example to add a Fedora 34 LXD container with DHCP networking to the 172.29.108.0/24 "sx1" "SeedNet" OpenvSwitch network run the follwing command as shown below.
...
If using the optional SCST Linux SAN fully-automated deployer which is found in /opt/olxc/home/scst-files AFTER Orabuntu-LXC deployment has finished, then first run "/opt/olxc/home/scst-files/create-scst.sh" (absolute path may differ based on location of the original Orabuntu-LXC install stage) and then when that completes successfully, use the following command to expose local host SCST Linux SAN LUNs to any given container. An example command is shown below.
