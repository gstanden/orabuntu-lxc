...
Note that if LXD=Y and LXDCluster=Y are selected, it will be necessary to first create the required ZFS storage pool for the LXD cluster.  The naming convention olxc-001 (for HUB node) and optionally olxc-00[2,3,4 ...] (for GRE node if creating N-node LXD Cluster) etc is suggested, but any naming arbitrary naming scheme can be used in the ./anylinux/CONFIG file for naming the ZFS storage pools. 
