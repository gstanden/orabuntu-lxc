...
Note 1: That in the following, this new update feature is only available on Oracle Linux 7 VM's at this time, but it should be ported very very soon to Ubuntu VM's as well. As far as physical hosts, ANY Orabuntu-LXC supported physical host can be used, notably Ubuntu Linux and Oracle Linux.  Only the VirtualBox VMs have to be Oracle Linux 7.  Support for Ubuntu VMs for this same feature will be released very soon (ed. see latest update, Ubuntu VM support has been added).
