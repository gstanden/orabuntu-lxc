clear
~/Downloads/orabuntu-lxc-master/ubuntu-services-1.sh 6 7 corp\.chartercom\.com kstllmcomd01
clear
~/Downloads/orabuntu-lxc-master/ubuntu-services-2.sh 6 7
clear
~/Downloads/orabuntu-lxc-master/ubuntu-services-3.sh 6 7
clear
~/Downloads/orabuntu-lxc-master/ubuntu-services-4.sh 6 7 10 kstlxc
clear
~/Downloads/orabuntu-lxc-master/ubuntu-services-5.sh 6 7

