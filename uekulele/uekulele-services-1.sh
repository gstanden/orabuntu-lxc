...
function CheckNsaRunning {
	sudo lxc-ls -f | grep nsa | sed 's/  */ /g' | cut -f2 -d' '
}
NsaRunning=$(CheckNsaRunning) > /dev/null 2>&1
...
then
	...
	echo ''
	echo "=============================================="
	echo "Done: LXC and related services started.       "
	echo "=============================================="
	echo ''
...
if [ ! -d /var/lib/lxc ]
then
        echo "/var/lib/lxc already exists" > /dev/null 2>&1
else
        sudo mkdir -p /var/lib/lxc
fi

function CheckNsaExists {
	sudo ls -l /var/lib/lxc | grep nsa | wc -l
}
NsaExists=$(CheckNsaExists)

function CheckNameServerExists {
	sudo ls -l /var/lib/lxc | grep $NameServer | wc -l
}
NameServerExists=$(CheckNameServerExists)

if   [ $NsaExists -eq 0 ] && [ $NameServerExists -eq 0 ]
then
	echo ''
	echo "=============================================="
	...
	then
		...
		if [ $LXCPackageCount -eq 5 ] && [ $Release -eq 7 ]
		then
			echo ''
			echo "=============================================="
		...
		if [ $LXCPackageCount -eq 4 ] && [ $Release -eq 6 ]
		then
			echo ''
			echo "=============================================="
			echo "LXC RPMs built on $LF Linux $RL.              " 
	...
	sudo yum -y install iptables gawk tunctl
	...
	if   [ $Release -eq 6 ] && [ $MultiHostVar2 = 'N' ]
	then
		echo ''
		echo "=============================================="
		...
		echo '' 
		echo "=============================================="
		echo "Set LXC DNS DHCP container to onboot start... "
		echo "=============================================="
		echo ''

		sudo sh -c "echo '# Autostart at boot' >> /var/lib/lxc/nsa/config"
		sudo sh -c "echo 'lxc.start.auto = 1'  >> /var/lib/lxc/nsa/config"
		sudo sh -c "echo 'lxc.group = onboot'  >> /var/lib/lxc/nsa/config"
		sudo sed -i '$!N; /^\(.*\)\n\1$/!P; D'    /var/lib/lxc/nsa/config 
		...
		echo '' 
		echo "=============================================="
		echo "Done: Set LXC DNS DHCP onboot start.          "
		echo "=============================================="
		echo ''
		...
		echo '' 
		echo "=============================================="
		echo "Reboot #1 required for $LF Linux $RL...       "
		echo "=============================================="
		echo ''
		echo "=============================================="
		echo "Re-run anylinux-services.sh after reboot...   "
	...
	elif [ $Release -eq 7 ] && [ $MultiHostVar2 = 'N' ]
	then
		echo ''
		echo "=============================================="
		echo "Create LXC DNS DHCP container...              "
	...
	if   [ $Release -eq 7 ]
	then
		sudo rm -f /etc/init.d/start_sw1
		sudo rm -f /etc/init.d/start_sx1
		sudo rm -f /etc/init.d/60-oracle
	fi
	...
	then
		...
		if [ ! -f /etc/systemd/system/60-oracle.service ] && [ $Release -eq 7 ]
		then
		...
		then
			echo ''
			echo "=============================================="
			echo "Set Onboot for $LF Linux $RL 60-oracle... "
			echo "=============================================="
			echo ''

			sudo chmod 755 /etc/init.d/60-oracle
			sudo sed -i "s/SHMMAX/$shmmax/g" /etc/init.d/60-oracle
			sudo sed -i "s/SHMALL/$shmall/g" /etc/init.d/60-oracle
			sudo chkconfig --add 60-oracle
			sudo service 60-oracle start
			...
			echo ''
			echo "=============================================="
			echo "Done: Set Onboot for $LF Linux $RL 60-oracle. "
			echo "=============================================="
...
elif [ $NsaExists -eq 1 ] && [ $NameServerExists -eq 0 ] && [ $NsaRunning = 'RUNNING' ]
then
	if [ $Release -eq 6 ]
	then
		echo ''
		...
		echo ''
		echo "=============================================="
		echo "Get DNS DHCP LXC container info...            "
		echo "=============================================="
		echo ''
		...
		echo ''
		echo "=============================================="
		echo "Done:  Get DNS DHCP LXC container info        "
		echo "=============================================="
		echo ''
	...
	function CheckOpenvSwitchSw1Exist {
		ifconfig | grep sw1 | wc -l
	}
	OpenvSwitchSw1Exist=$(CheckOpenvSwitchSw1Exist)

	sudo chmod 755 /etc/network/openvswitch/*.sh

	if   [ $Release -eq 7 ] && [ $OpenvSwitchSw1Exist -eq 0 ]
	then
	...
	elif [ $Release -eq 6 ] && [ $OpenvSwitchSw1Exist -eq 0 ]
	then
		...
		do
			...
			sudo cp -p /etc/init.d/sw1 /etc/init.d/$k > /dev/null 2>&1
			sudo sed -i "s/sw1/$k/g"   /etc/init.d/$k
	...
	function CheckNameServerRunning {
		sudo lxc-ls -f | grep $NameServer | sed 's/  */ /g' | cut -f2 -d' '
	}
	NameServerRunning=$(CheckNameServerRunning)
	...
	if [ ! -f /etc/systemd/system/$NameServer.service ] && [ $Release -eq 7 ]
	then
		echo ''
		echo "=============================================="
	...
	then
		...
		then
			...
			sudo service sw1 restart > /dev/null 2>&1
			sleep 2
			sudo service $NameServer restart > /dev/null 2>&1
			sleep 2
			nslookup $NameServer
			if [ $? -ne 0 ]
			then
		...
		echo ''
		echo "=============================================="
		echo "Done: Start LXC DNS.                          "
		echo "=============================================="
		echo ''

		fi

	elif [ $Release -eq 6 ] && [ $NameServerRunning = 'STOPPED' ]
	then
		echo '' 
		echo "=============================================="
		echo "Set LXC DNS DHCP container to onboot start... "
		...
		sudo sh -c "echo '# Autostart at boot' >> /var/lib/lxc/$NameServer/config"
		sudo sh -c "echo 'lxc.start.auto = 1'  >> /var/lib/lxc/$NameServer/config"
		sudo sh -c "echo 'lxc.group = onboot'  >> /var/lib/lxc/$NameServer/config"
		sudo sed -i '$!N; /^\(.*\)\n\1$/!P; D'    /var/lib/lxc/$NameServer/config
		...
		echo '' 
		echo "=============================================="
		echo "Reboot #2 required for $LF Linux $RL...       "
		echo "=============================================="
		echo ''
		echo "=============================================="
		echo "Re-run anylinux-services.sh after reboot...   "
